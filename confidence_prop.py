...
def main():
    ...
    result_folder = '/'.join(config.result_path.split('/')[:-1])
    result_filename = config.result_path.split('/')[-1]
    input_filename = config.input_text.split('/')[-1]
    config.result_path = result_folder + '/' + input_filename + '_' + args.algorithm + '_' + result_filename
